    ...
    return <div className="w-11/12 mx-auto bg-white shadow-md rounded-sm overflow-hidden mb-4">
        <div className="flex items-center justify-between p-2 justify-content-between">
                    ...
                    <input onClick={()=>{
                        ...
                        setFlag(flag+1)
                    }} checked={order.isCheck===true} type="checkbox"/>
                        <span className="font-semibold text-sm">{order.accountId.hoVaTen} - {order.accountId.soDienThoai} </span>
                        <span className="font-semibold text-sm">• DH{order.id} </span>
                    </div>
                    <span className="text-red-500 text-sm pr-2 font-semibold w-12/12">Chuẩn bị hàng</span>
                </div>
                <div className="flex items-center justify-between p-2 mb-3 justify-content-between">
                    <div className="flex items-center space-x-2 w-10/12">
                        <span className="font-semibold text-sm">{new Date(order.ngayTaoDon).toString}</span>
                    </div>
                    <span className="text-red-500 text-sm pr-2 font-semibold w-12/12">{order.thanhToanId.hinhThucThanhToan}</span>
                </div>
                <div className="w-[95%] h-[1px] bg-lightgray mx-auto"></div>
                {
