                            ...
                            >
                                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                    ...
                                    <div className="flex overflow-x-auto">
                                        ...
                                        {categories.map((v) => {
                                            return (
                                                <ul style={{ cursor: 'pointer' }} className="w-1/3 border-r border-gray-200">
                                                    {v.map((v) => (
                                                        <li className='p-2 hover:bg-gray-300' onClick={() => { setCChoose([v]); }}>
                                                            <div className='flex justify-content-between'>
                                                                <span>{v.tenLoai}</span>
                                                                <span className='text-md'> {v.subCategories.length > 0 ? '>' : ''}</span>
                                                            </div>
                                                         </li>
                                                    ))}
                                                </ul>
                                                ...
                                                <ul className="w-1/3 border-r border-gray-200" style={{ cursor: 'pointer' }}>
                                                    {v.subCategories.map((subCategory) => (
                                                        ...
                                                        >
                                                            <div className="flex justify-content-between">
                                                                <div className="flex justify-content-between">
                                                                    <span>{subCategory.tenLoai} </span>
                                                                    <span className="text-md">
                                                                        {' '}
                                                                        {subCategory.subCategories.length > 0 ? (
                                                                            <span onClick={() => addSub(subCategory, id)}> > </span>
                                                                        ) : (
                                                                            <spam></spam>
                                                                        )}
                                                                    </span>
                                ...
                                <div className="flex mt-2 ml-2 mr-2" style={{ alignItems: 'center', justifyContent: 'space-between' }}>
                                    ...
                                    <div className=" px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6" style={{ alignItems: 'center' }}>
                                        {cChooose.length > 0 && cChooose[cChooose.length - 1].subCategories.length === 0 ? (
                                            <button className="btn btn-danger" onClick={closeModal} type="button">
                                                Xác nhận{' '}
                                            </button>
                                        ) : (
                                            <></>
