...
const TabProduct = ({ setTab }) => {
    ...
    return (
        <div  className="rounded-[7px] container mx-auto p-4 relative bg-white">
            {/* Tabs */}
            <p className="font-semibold  text-sm sm:text-base text-xl">Kênh người bán &gt; Quản lý sản phẩm &gt; Xem sản phẩm</p>
                ...
                <nav className="flex space-x-8 relative " aria-label="Tabs">
                    {['1', '0', '2'].map((tabId, index) => (
                        <span 
                            ...
                            className={`text-base  tab-item text-gray-${activeTab === tabId ? '900' : '500'} cursor-pointer py-4 px-4 font-medium relative`}
