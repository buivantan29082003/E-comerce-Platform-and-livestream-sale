import { useContext, useEffect, useState } from 'react';
import KhuyenMaiModal from './ModalKhuyenMai';
import api from '../../../config/APISeller';
import ButtonAdd from '../utils/ButtonAdd';
// import ButtonDelete from '../utils/ButtonDelete';
import toast from 'react-hot-toast';
import formatToVND from '../../client/FormatVnd';
import { AppContext } from '../../../App';
import { useNavigate } from 'react-router-dom';
import { Button } from '@mui/material';
// import { Button } from '@mui/material';
// import { MdDelete } from 'react-icons/md';

const KhuyenMaiView = () => {
    let date = new Date();
    date.setHours(date.getHours() + 2);
    const [khuyenMai, setKhuyenMai] = useState({
        tenKhuyenMai: 'Khuyến mãi giá tốt',
        giaTriKhuyenMai: 2,
        ngayBatDau: new Date(),
        ngayKetThuc: date,
    });
    const navigate=useNavigate()
    const {setFloadingPage } = useContext(AppContext);
    const [listProduct, setListProduct] = useState(new Map());
    const [flag, setFlag] = useState(0);
    const handleSubmit = () => {
        if (listProduct.size > 0) {
            insertPromotion();
        } else {
            alert('Bạn chưa chọn sản phẩm nào ');
        }
    };

    useEffect(()=>{
        checkValidProduct()
    },[flag])

    const checkValidProduct=()=>{
        api.post("/khuyenmai/checkvalid", {
            "startDate":khuyenMai.ngayBatDau,
            "endDate":khuyenMai.ngayKetThuc,
            "l":Array.from(listProduct.keys())
        },{
            headers: {
                'Content-Type': 'application/json',
            }
        }).then(v=>v.data).then(v=>{
            if(v.length>0){
                toast('Có một số sản phẩm không hơp lệ cần loại bỏ', {
                    icon: '⚠️',
                  });
                v.forEach(vv => {
                    listProduct.delete(vv)
                });
                setFlag(flag+1)
            }
        })
    }

    const deleteItem=(id)=>{
        listProduct.delete(id)
        setFlag(flag+1)
    }


    const insertPromotion = () => {
            toast.promise(
                api.post(
                    '/khuyenmai/insert',
                    {
                        khuyenMai: khuyenMai,
                        set: Array.from(listProduct.keys()),
                    },
                    {
                        headers: {
                            'Content-Type': 'application/json',
                        },
                    },
                )
                    .then((v) => v.data)
                    .then((v) => {
                        if(v.status!==200){
                            throw new Error(v.message)
                        }else{
                            navigate("/seller/khuyenmai")
                        }
                    }),
                {
                    
                  loading: "Creating khuyến mãi.....", 
                  success: 'Create khuyến mãi successfully.', 
                  error: (error) => error.message,
                }
              );
    };

    useEffect(()=>{
        setFloadingPage()
    },[])

    return (
        <>
            <div style={{ borderRadius: '7px' }} className="container mx-auto p-4 relative bg-white">
                <p className=" font-semibold">Kênh người bán * Quản lý khuyến mãi </p>
                <div className=" border-gray-200 relative"></div>
            </div>

            <div style={{ borderRadius: '7px' }} className="shadow-md mt-3 container mx-auto p-4 relative bg-white">
                <p className="font-semibold">Thông tin cơ bản</p>
                <div class="container mx-auto mt-3">
                    <div class="grid grid-cols-12 gap-4">
                        <div class="col-span-12 lg:col-span-2 text-md   lg:text-right sm:text-left">Tên khuyến mãi</div>

                        <div class="col-span-12 lg:col-span-10  ">
                            <input
                                value={khuyenMai.tenKhuyenMai}
                                onChange={(e) => {
                                    khuyenMai.tenKhuyenMai = e.target.value;
                                    setFlag(flag + 1);
                                }}
                                type="text"
                                placeholder="Tên khuyến mãi"
                                class="w-11/12 lg:w-10/12 p-2 border border-gray-200 rounded-sm focus:outline-none focus:ring-1 focus:ring-blue-400 text-sm"
                            />
                            <p className="hidden lg:block text-xs mt-2 text-danger">
                                {khuyenMai.tenKhuyenMai.length < 1 || khuyenMai.tenKhuyenMai.length > 145
                                    ? 'Độ dài tên khuyến mãi không hợp lệ'
                                    : ''}
                            </p>
                            <p className="hidden lg:block text-xs mt-2 text-warning"></p>
                        </div>
                    </div>
                </div>

                <div class="container mx-auto mt-3">
                    <div class="container mx-auto mt-3">
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-12 lg:col-span-2 text-md  lg:text-right sm:text-left">Giá trị giảm</div>
                            <div class="col-span-12 lg:col-span-10  ">
                                <input 
                                    value={khuyenMai.giaTriGiam}
                                    onChange={(e) => {
                                        khuyenMai.giaTriKhuyenMai = e.target.value;
                                        setFlag(flag + 1);
                                    }}
                                    type="number"
                                    placeholder="Giá trị giảm"
                                    class="w-11/12 lg:w-10/12 p-2 border border-gray-200 rounded-sm focus:outline-none focus:ring-1 focus:ring-blue-400 text-sm"
                                />
                                <p className="hidden lg:block text-xs mt-2 text-danger">
                                    {khuyenMai.giaTriKhuyenMai < 1 || khuyenMai.giaTriKhuyenMai > 99 ? 'Giá trị giảm không hợp lệ' : ''}
                                </p>
                                <p className="hidden lg:block text-xs mt-2 text-warning">Từ 1-99</p>
                            </div>
                        </div>
                    </div>
                </div>

                {/* thời gian sử dụng voucher */}
                <div class="container mx-auto mt-3">
                    <div class="grid grid-cols-12 gap-4">
                        <div class="col-span-12 lg:col-span-2 text-md  lg:text-right sm:text-left">Thời gian sử dụng mã </div>

                        <div class="col-span-12 lg:col-span-10">
                            <input
                                onChange={(e) => {
                                    khuyenMai.ngayBatDau = new Date(e.target.value);
                                    setFlag(flag+1)
                                    checkValidProduct()
                                }}
                                type="datetime-local"
                                placeholder="Tìm Tên sản phẩm, SKU sản phẩm, SKU phân loại, Mã sản phẩm"
                                class="w-5/12 lg:w-4/12 p-2 border border-gray-200 rounded-sm focus:outline-none focus:ring-1 focus:ring-blue-400 text-sm"
                            />
                            <span className="inline-block ml-1 ml-2"> to </span>
                            <input
                                onChange={(e) => {
                                    khuyenMai.ngayKetThuc = new Date(e.target.value);
                                    checkValidProduct()
                                }}
                                type="datetime-local"
                                placeholder="Tìm Tên sản phẩm, SKU sản phẩm, SKU phân loại, Mã sản phẩm"
                                class="ml-2 w-5/12 lg:w-4/12 p-2 border border-gray-200 rounded-sm focus:outline-none focus:ring-1 focus:ring-blue-400 text-sm"
                            />
                            <p class="hidden lg:block text-xs mt-2 text-warning">
                            </p>
                        </div>
                    </div>
                </div>
            </div>

            <div style={{ borderRadius: '7px' }} className="mt-2 container mx-auto p-4 relative bg-white">
                <div class="flex justify-between items-center w-full p-2 flex-wrap md:flex-nowrap">
                    <div class="text-gray-500">
                        <span className="text-sm">
                            Sản phẩm được áp dụng ( <span className="text-xs ">1 sản phẩm được áp dụng</span> )
                        </span>
                    </div>
                    <KhuyenMaiModal khuyenMai={khuyenMai} map={listProduct} flag={flag} setFlag={setFlag} />
                </div>
            </div>

            <div style={{ borderRadius: '7px', overflow: 'auto' }} className="mt-2 container mx-auto p-4 relative bg-white">
                <table className="rounded-sm min-w-full bg-white border border-gray-200" style={{ overflow: 'hidden' }}>
                    <thead className="bg-gray-700 p-2 ">
                        <tr className='text-white'>
                            <th className="px-4 py-2 border-b text-left font-semibold "> </th>
                            <th className="px-4 py-2 border-b text-left font-semibold ">Tên sản phẩm</th>
                            <th className="px-4 py-2 border-b text-left font-semibold ">Giá gốc</th>
                            <th className="px-4 py-2 border-b text-left font-semibold">Giá sau giảm</th>
                            <th className="px-4 py-2 border-b text-left font-semibold ">Kho hàng</th>
                            <th className="px-4 py-2 border-b text-left font-semibold "></th>
                        </tr>
                    </thead>
                    <tbody className="text-xs">
                        {/* Row */}
                        {Array.from(listProduct.entries()).map(([key, value]) => {
                            return (
                                <>
                                    <tr>
                                        <td className="text-center "></td>
                                        <td className="px-4 py-4">
                                            <div className="flex items-center ">
                                                <img className='mr-1' src={value.productImage} style={{ width: '70px' }} />

                                                <div>
                                                    <div className="font-semibold text-md text-gray-800">{value.productName}</div>
                                                    <div className="text-md text-gray-500 mt-2">ID Sản phẩm: {value.productId}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 border-b text-gray-700"></td>
                                        <td className="px-4 py-4 border-b text-gray-700"></td>
                                        <td className="px-4 py-4 border-b text-blue-700 font-medium">{value.totalProduct} sản phẩm</td>
                                        <td className="px-4 py-4 border-b text-gray-700" onClick={()=>deleteItem(key)}>
                                        <button type="button" data-modal-target="delete-modal" data-modal-toggle="delete-modal" class="flex items-center text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-2 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 -ml-0.5" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                            <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                                        </svg>
                                        Delete
                                    </button></td>
                                    </tr>
                                    {value.productDetails.map((v) => {
                                        return (
                                            <>
                                                <tr className='bg-gray-400'>
                                                    <td></td>
                                                    <td className="px-4 py-4" >
                                                        <div className="flex items-center ">
                                                            <img
                                                                style={{ width: '50px' }}
                                                                src="data:image/png;base64,"
                                                                alt="Product Image"
                                                                class="w-10 h-10 mr-4"
                                                            />
                                                            <div>
                                                                <div className="font-semibold text-white">
                                                                    {v.mauSac.tenMau} - {v.kichThuoc.tenMau}
                                                                </div>
                                                                <div className="text-sm text-white">ID biến thể: {v.id}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td  className="px-4 py-4 text-white">
                                                        {formatToVND(v.giaBan)}
                                                    </td>
                                                    <td  className="px-4 py-4 text-white">
                                                        {formatToVND(v.giaBan - v.giaBan * (khuyenMai.giaTriKhuyenMai / 100))}
                                                    </td>
                                                    <td  className="px-4 py-4 text-blue-700 font-medium">
                                                        {v.soLuong} sản phẩm
                                                    </td>
                                                    <td  className="px-4 py-4 text-gray-700">
                                                    </td>
                                                </tr>
                                            </>
                                        );
                                    })}
                                </>
                            );
                        })}
                    </tbody>
                </table>
            </div>
            <div
                onClick={() => {
                    handleSubmit();
                }}
                style={{ position: 'fixed', bottom: '20px', right: '8%' }}
            >
                <Button variant="contained">Thêm khuyến mãi</Button>
            </div>
        </>
    );
};
export default KhuyenMaiView;


