...
const OrderDetail=()=>{
    ...
    <p class="text-sm text-gray-500 mb-4">Ngày tạo đơn: {format(new Date(order.ngayTaoDon), 'HH:mm dd/MM/yyyy')} </p>
    <p class="text-sm text-gray-500 mb-4">Tên khách hàng: {order.accountId.hoVaTen} - {order.accountId.soDienThoai} </p>
    <p class="text-sm text-gray-500 mb-4">Địa chỉ người nhận: {order.diaChiId.toanBoDiaChi} </p>
    <Box sx={{ width: '100%' }}>
      <Stepper 
    ...
    <div class="mt-4 border rounded-lg p-4 mb-6">
        ...
        {order.orderDetails.map(v=>{
            ...
            <div>
                <h6 class="font-medium">{v.product.product.tenSanPham}</h6>
                <div class="flex items-center text-sm text-gray-600">
                    <span>Phân loại: {v.product.mauSac.tenMau} </span>  -
                    <span> {v.product.kichThuoc.tenKichThuoc}</span>
                    {/* <span class="w-3 h-3 bg-black rounded-full ml-2"></span> */}
                </div>
            </div>
            <div class="ml-auto text-right">
                <p>{v.soLuong} x {formatToVND(v.giaBan)}</p>
                <p class="font-semibold">{formatToVND(v.soLuong*v.giaBan)}</p>
            </div>
        </div>
        })}
    ...
    <div class="border rounded-lg p-4 mb-6">
        ...
        <div class="space-y-2 text-sm">
            <div class="flex justify-between">
                <span>Phí ship</span>
                <span>{formatToVND(order.phiShip)}</span>
            </div>
            <div class="flex justify-between">
                <span>Tiền voucher shop</span>
                {/* <span>{order.orderDetails} item</span> */}
                <span>{formatToVND(order.tienTru)}</span>
            </div>
            <div class="flex justify-between">
                <span>Tiền voucher sàn</span>
                {/* <span>{order.orderDetails} item</span> */}
                <span>{formatToVND(order.tienTruVoucherSan)}</span>
            </div>
            <div class="flex justify-between">
                <span>Tổng cộng</span>
                {/* <span>{order.orderDetails} item</span> */}
                <span>{formatToVND(order.phiShip+order.tongTien-order.tienTruVoucherSan-order.tienTru)}</span>
            </div>
        </div>
