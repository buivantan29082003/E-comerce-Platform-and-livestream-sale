...
// Lazy load ModalCancel
const ModalCancel = React.lazy(() => import("./ModalCancel"));

const OrderItemChuanBi = ({ order, removeOrder, index }) => {
            ...
            <tr className="hover:bg-gray-100 transition-colors">
                ...
                <td className="px-4 py-4 max-w-[200px] text-sm">
                    <span className="text-green-600 inline-flex items-center justify-center bg-green-100 px-2 py-1 rounded-full text-xs font-medium shadow-sm">
                        <FaCheckCircle className="mr-1" /> {'Chuẩn bị hàng'}
                    </span>
                </td>
                ...
                <td className="px-4 py-4 max-w-[200px] truncate text-sm">
                    <div>
                        ...
                        >
                            <MenuItem > <Link to={"/orderdetail/"+order.id}>  Xem chi tiết</Link></MenuItem>
                            <MenuItem onClick={updateOrderSingle}>Tiếp tục xử lý</MenuItem>
                            <MenuItem>
                                <Suspense fallback={<div>Loading...</div>}>
                                    <ModalCancel index={index} reload={removeOrder} id={order.id} />
                                </Suspense>
                            </MenuItem>
                        </Menu>
                    </div>
