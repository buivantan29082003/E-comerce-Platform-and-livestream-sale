...
public interface ProductRepository extends JpaRepository<Product,Integer>{
	...
	@Query("SELECT new com.API.model.Product(p.id, p.tenSanPham, p.hinhAnh, p.trangThai) " +
		       "FROM Product p " +
		...
		org.springframework.data.domain.Page<Product> getProductNotInlive(
		        @Param("id") Integer id,  // tham số cần phải có khi gọi API
		        Pageable p,
...
@Query(value = "WITH RECURSIVE subcategories AS ( " +
        "    SELECT id " +
        "    FROM theloai " +
        "    WHERE (COALESCE(:theLoaiIds) IS NULL OR parent_Id IN (:theLoaiIds)) " +
        "    UNION ALL " +
        "    SELECT t.id " +
        "    FROM theloai t " +
        "    JOIN subcategories s ON s.id = t.parent_Id " +
        ") " +
        "SELECT " +
        "    sp.id AS sanPhamId, " +
        "    sp.tenSanPham AS tenSanPham, " +
...
"ORDER BY sp.id DESC",
countQuery = "WITH RECURSIVE subcategories AS ( " +
        "    SELECT id " +
        "    FROM theloai " +
        "    WHERE (COALESCE(:theLoaiIds) IS NULL OR parent_Id IN (:theLoaiIds)) " +
        "    UNION ALL " +
...
"WHERE sp.trangThai = 1 " +
        ...
        "GROUP BY sp.id, sp.tenSanPham, sp.hinhAnh, km.giaTriKhuyenMai, sp.soLuongDaBan",
nativeQuery = true)
Page<Object[]> getAndFindPromotionalProducts(
...
@Param("giaTu") Double giaTu,
@Param("giaDen") Double giaDen,
Pageable pageable);
...
//		 Integer (@Param("productId") Integer productId,@Param("shopId") Integer shopId);
		 ...
		 @Query(value = "SELECT " +
			        ...
			        "    p.tenSanPham, " +
			        "    p.hinhAnh, " +
			        "    COUNT(DISTINCT d.id) AS tongDon, " +
			        "    COUNT(DISTINCT d.taiKhoanId) AS soNguoiMua, " +
			        "    SUM(c.soLuong) AS tongSoLuongDaBan, " +
			        "    SUM(d.tongTien) AS tongTien " +
			        "FROM " +
			        "    chitietdonhang c " +
			        "JOIN " +
			        "    donhang d ON d.id = c.donHangId " +
			        "JOIN " +
			        "    chitietsanpham pd ON pd.id = c.productId " +
			        "JOIN " +
			        "    sanpham p ON p.id = pd.sanPhamId " +
			        "WHERE " +
			        "    d.shopId = :shopId " +
			        "GROUP BY " +
			        "    p.id, p.tenSanPham, p.hinhAnh " +
			        "HAVING " +
			        "    SUM(c.soLuong) > 9 " +
		 ...
		 @Query("SELECT COUNT(DISTINCT d.product.id) " +
			       "FROM ProductDetail d where d.product.shop.shopId=:shopId " +
			       "GROUP BY d.product.id " +
			       "HAVING SUM(d.soLuong) < 10")
...
//	    Integer id,String tenMau,String tenKichThuoc,String hinhAnh
	    ...
	    Integer updateStatusByIdsAndShopId(@Param("trangThaiId") Integer trangThaiId, @Param("ids") List<Integer> ids, @Param("shopId") Integer shopId);
	    ...
	    @Query("SELECT new com.API.model.Product(p.id, p.tenSanPham, p.hinhAnh, p.trangThai) " +
	    	       ...
	    	       "")
	    	org.springframework.data.domain.Page<Product> findAllProductsWithDetails(
	    	        @Param("id") Integer id,
	    	        Pageable p,
	    ...
	    @Query("SELECT new com.API.model.Product(p.id, p.tenSanPham, p.hinhAnh, p.trangThai) " +
	    	       ...
	    	       "") 
	    	org.springframework.data.domain.Page<Product> findAllProductsWithDetailsAndIds(
	    	        @Param("id") Integer id,
	    	        Pageable p,
	    	        @Param("state") Integer state,
	    	        @Param("key") String key,
	    ...
	    org.springframework.data.domain.Page<Product> findAllProductsWithDetails(@Param("id")Integer id,@Param("status")Integer status,Pageable p);
...
"    sp.soLuongDaBan AS soLuongDaBan " +
                ...
                "GROUP BY sp.id, sp.tenSanPham, sp.hinhAnh, km.giaTriKhuyenMai, sp.soLuongDaBan",
        nativeQuery = true)
Page<Object[]> findSanPhamWithFilters(
