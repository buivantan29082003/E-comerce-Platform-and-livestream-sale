...
public interface LiveSessionRepository extends JpaRepository<LiveSession, Integer> {
	...
	@Query("select s.id from LiveSession s where s. startTime is not null and s.endTime is null and s.shop.shopId =:shopId")
	Integer getShopIsLive(@Param("shopId")Integer shopId);
	...
	@Transactional
	@Modifying
	@Query("update Live_Product s set s.soLuongGioiHan=s.soLuongGioiHan-:soLuong where s.id=:id")
	Integer updateCountPromotionById(@Param("id")Integer id,@Param("soLuong")Integer soLuong);
...
//	                            @Param("giaGiam") Integer giaGiam);
	...
	@Modifying
	@Transactional
	@Query("UPDATE LiveSession p set p.countLike=:countLike, p.cartCount=:cartCount,p.endTime=now()  WHERE p.id = :id")
